...
import gilcu2.url_shortener.encoder.HashidsEncoder
...
import gilcu2.url_shortener.id_generator.{BlockManager, IdGenerator}
import gilcu2.url_shortener.storage
...
class MyShortenerTest extends AnyFlatSpec with GivenWhenThen with BeforeAndAfterAll {
  ...
  val redis: storage.Redis = storage.Redis()
